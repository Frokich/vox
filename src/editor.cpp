...
public:
    Editor(const std::string& fname) : filename(fname) {
        config.load(std::string(getenv("HOME"))+"/.config/vox/init.lua");
    }
...
private:
    ...
    void pushUndo() {
        EditorState state{lines, cursorX, cursorY};
        ...
        if(mode == NORMAL){
            ...
            switch(ch){
                ...
                case 'o':
                    ...
                    cursorY++; cursorX=0; mode=INSERT; break;
                case 'O':
                    pushUndo();
                    lines.insert(lines.begin()+cursorY,"");
                    cursorX=0; mode=INSERT; break;
                case 'x':
                    if(cursorX<lines[cursorY].size()){ pushUndo(); lines[cursorY].erase(cursorX,1); }
                    break;
                case 'd':
                    if(dPressed){
                        pushUndo();
            ...
            switch(ch){
                ...
                case '\t':
                    pushUndo();
                    if(config.expandTab){
                        lines[cursorY].insert(cursorX, config.tabWidth, ' ');
                        cursorX += config.tabWidth;
                    } else {
                        lines[cursorY].insert(cursorX,1,'\t'); cursorX++;
                    }
                    break;
            ...
            if(ch==10){ // Enter
                ...
                if(!cmdBuffer.empty()) cmdBuffer.pop_back();
            } else if(ch>=32 && ch<=126){
                cmdBuffer.push_back(ch);
